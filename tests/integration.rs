...
fn test_channel_lookup_by_name() {
    ...
    for ch in &s.channels {
        assert!(
            s.channel(&ch.name).is_some(),
            "channel '{}' not findable by name",
            ch.name
        );
    }
}
...
fn test_samples_have_valid_timestamps() {
    ...
    for ch in &s.channels {
        for sample in &ch.samples {
            assert!(
                sample.time_sec >= 0.0,
                "negative timestamp in channel '{}'",
                ch.name
            );
            assert!(
                sample.time_sec < 10_000.0,
                "unreasonably large timestamp in channel '{}'",
                ch.name
            );
        }
    }
...
fn test_samples_sorted_by_time() {
    ...
    for ch in &s.channels {
        ...
        for w in times.windows(2) {
            assert!(
                w[0] <= w[1],
                "channel '{}' samples not in time order",
                ch.name
            );
        }
    }
...
fn test_two_point_calibration() {
    ...
    assert!(
        (result_low - 0.0).abs() < 0.5,
        "expected ~0mm, got {:.3}",
        result_low
    );
    assert!(
        (result_high - 50.0).abs() < 0.5,
        "expected ~50mm, got {:.3}",
        result_high
    );
}
...
fn test_logger_config_apply() {
    use xrk::config::{Calibration, LoggerConfig};
